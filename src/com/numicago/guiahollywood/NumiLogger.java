...
public class NumiLogger {
	private static NumiLogger instance = null;
	...
	public NumiLogger(Context context) {
		LoggerSetup(context.getPackageName(), "application.log", Level.INFO);
	}

	public static NumiLogger getInstance(Context context) {
		if(instance == null) {
			instance = new NumiLogger(context);
		}
		return instance;
	}
	...
	private File createWriter(String logFilename) {
		try {
			...
			if( state.equals(Environment.MEDIA_MOUNTED) ) {
				File dir = new File( Environment.getExternalStorageDirectory(), "CanalHollywood/LogFiles");
				if( !dir.mkdirs() ) {
